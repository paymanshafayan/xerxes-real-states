...
- **GitHub Actions Workflows added & extended**:
  ...
  - Admin APK Upload (`AppDownloadsManager.tsx` & `/api/admin/app-downloads/upload`): Added automatic fallback to local storage (`public/uploads/apps/...`) in `src/lib/appStorage.ts` when R2 is not configured; configured `runtime = "nodejs"` and `maxDuration = 300` on the upload route and `bodySizeLimit: "200mb"` in `next.config.ts`; upgraded UI with real-time percentage progress bar (`xhr.upload.onprogress`), disabled upload button while uploading, and spinner icon.
  - Admin Help Guide (`AdminHelpGuide.tsx`): Fixed interactive section navigation (`activeSection` state) so clicking any topic opens its guide; added full step-by-step documentation for `user_listings`, `visit_requests`, `blocked_users`, and `app_downloads`; added "← Back to all topics" navigation.
