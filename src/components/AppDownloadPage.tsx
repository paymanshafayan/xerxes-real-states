...
export default function AppDownloadPage({ config }: { config: AppDownloadConfig }) {
  ...
  const copy = {
    en: { eyebrow: "Xerxes for customers", title: "Find your next property, wherever you are.", description: "Browse verified homes in Northern Cyprus, save the ones you love and talk to an expert when you are ready.", apk: "Direct Android download", unavailable: "Download links will appear here as soon as the app is released.", scan: "Scan to download Android", features: [["360° virtual tours", "Explore supported properties before travelling."], ["Price-drop alerts", "Keep up with genuine reductions on saved homes."], ["Expert live chat", "Speak to a consultant in English, Turkish, Persian or Russian."]] },
    tr: { eyebrow: "Müşteriler için Xerxes", title: "Nerede olursanız olun yeni evinizi bulun.", description: "Kuzey Kıbrıs'taki doğrulanmış evleri inceleyin, beğendiklerinizi kaydedin ve hazır olduğunuzda bir uzmanla konuşun.", apk: "Android'i doğrudan indir", unavailable: "Uygulama yayınlandığında indirme bağlantıları burada görünecek.", scan: "Android indirmek için tarayın", features: [["360° sanal turlar", "Desteklenen mülkleri seyahat etmeden önce keşfedin."], ["Fiyat düşüşü uyarıları", "Kaydedilen evlerdeki gerçek indirimleri takip edin."], ["Uzman canlı sohbet", "İngilizce, Türkçe, Farsça veya Rusça danışmanla konuşun."]] },
    fa: { eyebrow: "Xerxes برای مشتریان", title: "خانه بعدی‌تان را از هر جا پیدا کنید.", description: "خانه‌های تأییدشده در قبرس شمالی را ببینید، موارد دلخواه را ذخیره کنید و در زمان مناسب با کارشناس گفت‌وگو کنید.", apk: "دانلود مستقیم اندروید", unavailable: "لینک‌های دانلود پس از انتشار اپلیکیشن در این بخش نمایش داده می‌شوند.", scan: "برای دانلود اندروید اسکن کنید", features: [["تورهای مجازی ۳۶۰ درجه", "پیش از سفر، املاک پشتیبانی‌شده را بررسی کنید."], ["هشدار کاهش قیمت", "کاهش قیمت‌های واقعی املاک ذخیره‌شده را پیگیری کنید."], ["چت زنده با کارشناس", "با مشاور به فارسی، انگلیسی، ترکی یا روسی گفت‌وگو کنید."]] },
    ru: { eyebrow: "Xerxes для клиентов", title: "Найдите новый дом, где бы вы ни были.", description: "Изучайте проверенные объекты на Северном Кипре, сохраняйте понравившиеся и общайтесь с экспертом, когда будете готовы.", apk: "Скачать Android напрямую", unavailable: "Ссылки на скачивание появятся здесь после выпуска приложения.", scan: "Отсканируйте для Android", features: [["Виртуальные туры 360°", "Осматривайте поддерживаемые объекты до поездки."], ["Уведомления о снижении цены", "Следите за реальными снижениями по сохранённым объектам."], ["Чат с экспертом", "Общайтесь с консультантом на английском, турецком, персидском или русском."]] },
  }[locale];
  const apkAbsoluteUrl = config.apkUrl ? `${origin}${config.apkUrl}` : "";
  ...
  return <section className="min-h-[calc(100vh-200px)] bg-gradient-to-b from-slate-50 to-white py-12 sm:py-16"><div className="max-w-6xl mx-auto px-4 sm:px-6">
    <div className="grid lg:grid-cols-[1.1fr_.9fr] gap-8 items-stretch">
      ...
      <aside className="rounded-3xl bg-white border border-gray-200 shadow-xl shadow-slate-200/40 p-7 sm:p-8 flex flex-col">
        <div className="flex items-start justify-between gap-4"><div><div className="w-12 h-12 rounded-2xl bg-primary-light flex items-center justify-center"><Smartphone className="w-6 h-6 text-primary" /></div><h2 className="text-xl font-bold text-gray-900 mt-4">{dict.nav.app}</h2></div>{config.apkUrl && <div className="text-center"><img className="w-24 h-24 rounded-lg border border-gray-100" alt={copy.scan} src={`https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(apkAbsoluteUrl)}`} /><p className="text-[10px] text-gray-500 mt-1 max-w-24">{copy.scan}</p></div>}</div>
