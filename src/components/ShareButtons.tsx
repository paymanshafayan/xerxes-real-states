    ...
    <div className="relative">
      ...
      {showMenu && (
        <>
          ...
          />
          <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-200 py-2 z-20 animate-fade-in">
            {shareLinks.map((link) => (
              <a
                key={link.name}
                href={link.url}
                ...
                onClick={() => setShowMenu(false)}
                className={`flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:text-white ${link.color} transition-colors`}
              >
                <link.icon />
                {link.name}
              </a>
