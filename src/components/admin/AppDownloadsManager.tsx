...
export default function AppDownloadsManager() {
  ...
  const addStore = async () => {
    ...
    const next = {
      ...configs[active],
      stores: [...configs[active].stores, { id: crypto.randomUUID(), label, url, platform: draft.platform }],
    };
    await save(next);
  ...
  const removeStore = async (id: string) => {
    await save({ ...configs[active], stores: configs[active].stores.filter((store) => store.id !== id) });
  };
  ...
  const uploadApk = async (file?: File) => {
    ...
    try {
      ...
      const body = await new Promise<any>((resolve, reject) => {
        ...
        xhr.open("POST", "/api/admin/app-downloads/upload");
        if (token) {
        ...
        xhr.onload = () => {
          try {
            ...
            if (xhr.status >= 200 && xhr.status < 300) {
              ...
              reject(new Error(resBody.error || `Upload failed (Status ${xhr.status})`));
            }
          } catch {
            reject(
              new Error(
                `Server returned status ${xhr.status}. The file may be too large or the upload timed out.`
              )
            );
          }
        };
        xhr.onerror = () =>
          reject(
            new Error(
              "Network error during APK upload. Check your connection or server file size limit (max 200 MB)."
            )
          );
        xhr.send(form);
      });
      ...
      if (current.apkKey && current.apkKey !== body.key) {
        await adminFetch("/api/admin/app-downloads/upload", {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
    ...
    <div className="max-w-4xl space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Manage separate customer and staff distribution links. Only configured buttons appear publicly.
        </p>
      </div>
      ...
      <section className="bg-white border border-gray-200 rounded-2xl p-5 space-y-4">
        ...
        <div className="space-y-2">
          <label
            className={`inline-flex items-center gap-2 px-4 py-2.5 bg-primary text-white text-sm font-semibold rounded-lg transition-colors ${
              uploading
      ...
      <section className="bg-white border border-gray-200 rounded-2xl p-5 space-y-5">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Add any store. Each saved link appears as its own button on the appropriate download page.
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-[160px_1fr_150px_auto] gap-2">
          <select
            value={draft.platform}
            onChange={(e) =>
              setDraft((old) => ({ ...old, platform: e.target.value as StorePlatform }))
            }
          ...
          <input
            value={draft.label}
            onChange={(e) => setDraft((old) => ({ ...old, label: e.target.value }))}
            placeholder="Optional button label"
            className="border rounded-lg px-3 py-2 text-sm"
          />
          <input
            value={draft.url}
            onChange={(e) => setDraft((old) => ({ ...old, url: e.target.value }))}
            placeholder="https://…"
            className="border rounded-lg px-3 py-2 text-sm"
          />
          <button
            onClick={addStore}
            disabled={saving}
            className="inline-flex justify-center items-center gap-1 bg-gray-900 text-white rounded-lg px-3 py-2 text-sm"
          >
        ...
        <div className="space-y-2">
          {config.stores.length === 0 ? (
            <p className="text-sm text-gray-500">No store links configured.</p>
          ) : (
      ...
      {saving && (
        <div className="fixed bottom-5 right-5 bg-gray-900 text-white text-sm rounded-lg px-4 py-2">
          <Save className="inline w-4 h-4 mr-1" /> Saving…
        </div>
      )}
    </div>
  );
